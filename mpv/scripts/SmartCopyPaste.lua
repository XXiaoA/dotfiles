...
if not o.device or o.device == "auto" then
    ...
    elseif
        ...
        or os.execute('[ -d "/Applications" ]') == true
            and os.execute('[ -d "/Library" ]') == true
    then
        o.device = "mac"
    else
        o.device = "linux"
...
function format_time(seconds, sep, decimals, style)
    ...
    if not style or style == "" or style == "default" then
        local second_format =
            string.format("%%0%d.%df", 2 + (decimals > 0 and decimals + 1 or 0), decimals)
        sep = sep and sep or ":"
            ...
            for c_protocols in c:gmatch("[^%s]+") do --3.2# loop iterator using space
                if starts_protocol(protocols, c_protocols) then --3.2# check if it starts with protocols again after a space
                    ...
                    if string.match(c, "(.*)" .. c_pre_attribute) then
                        ...
                        c_clip_time =
                            tonumber(string.match(c_protocols, c_pre_attribute .. "(%d*%.?%d*)"))
        ...
        if action == "timestamp" then
            ...
            set_clipboard(
                pre_attribute
                    .. format_time(
                        video_time,
                        o.copy_time_format[3],
                        o.copy_time_format[2],
                        o.copy_time_format[1]
                    )
                    .. after_attribute
            )
            msg.info(
...
function trigger_paste_action(action)
    ...
    if action == "error-unsupported" then
        if o.osd_messages == true then
            mp.osd_message(
                "Paste of this item is unsupported possibly due to configuration:\n" .. clip
            )
        end
        msg.info(
...
function multipaste()
    ...
    if filePath == nil then
        for i = 1, #clip_table do
            if
                file_exists(clip_table[i][1])
                    and has_value(o.paste_extensions, clip_table[i][3])
    ...
    if filePath ~= nil then
        for i = 1, #clip_table do
            ...
            elseif
                file_exists(clip_table[i][1]) and has_value(o.paste_subtitles, clip_table[i][3])
            then
...
function paste()
    ...
    if #clip_table > 1 then
        ...
        if filePath == nil then
            ...
            elseif
                file_exists(clip_file) and has_value(o.paste_subtitles, currentVideoExtension)
            then
            ...
            elseif o.running_paste_behavior == "playlist" then
                if
                    ...
                    or filePath == clip_file
                        and starts_protocol(o.paste_protocols, clip_file)
                        and clip_time == nil
                then
