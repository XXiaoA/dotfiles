        ...
        elseif element.type == "slider" then
            ...
            if not (element.slider.markerF == nil) and (slider_lo.gap > 0) then
                ...
                for _, marker in pairs(markers) do
                    if
                        (marker >= element.slider.min.value)
    ...
    for n = 1, #elements do
        ...
        if element.eventresponder and (state.active_element == n) then
            ...
            if mouse_hit(element) then
                ...
                if
                    element.softrepeat
                    and (state.mouse_down_counter >= 15 and state.mouse_down_counter % 5 == 0)
                then
        ...
        elseif element.type == "button" then
            ...
            -- 认为1个中文字符约等于1.5个英文字符
            local charcount = (
                buttontext:len() + select(2, buttontext:gsub("[^\128-\193]", "")) * 2
            ) / 3
            if not (maxchars == nil) and (charcount > maxchars) then
                ...
                if charcount > limit then
                    while charcount > limit do
                        ...
                        charcount = (
                            buttontext:len()
                            + select(2, buttontext:gsub("[^\128-\193]", "")) * 2
                        ) / 3
                    end
...
function render_message(ass)
    if
        state.message_hide_timer
        and state.message_hide_timer:is_enabled()
...
function render()
    ...
    if state.initREQ then
        ...
        if
            ...
            and not (mouseX == nil or mouseY == nil)
        then
    ...
    if what == "down" or what == "press" then
        for n = 1, #elements do
            if
                ...
                and (
                    elements[n].eventresponder[source .. "_up"]
                    or elements[n].eventresponder[action]
                )
            then
                if what == "down" then
...
function tick()
    ...
    elseif
        (state.fullscreen and user_opts.showfullscreen)
        or (not state.fullscreen and user_opts.showwindowed)
    then
        -- render the OSC
        render()
    else
