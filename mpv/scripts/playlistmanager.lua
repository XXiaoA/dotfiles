      ...
      int StrCmpLogicalW(const wchar_t * psz1, const wchar_t * psz2);
        ...
        function MultiByteToWideChar(MultiByteStr)
            ...
            if MultiByteStr then
                local utf16_len =
                    ffi.C.MultiByteToWideChar(UTF8_CODEPAGE, 0, MultiByteStr, -1, nil, 0)
                if utf16_len > 0 then
...
function stripfilename(pathfile, media_title)
    ...
    if settings.filename_replace and not media_title then
        for k, v in ipairs(settings.filename_replace) do
            if
                replace_table_has_value(ext, v["ext"])
                or replace_table_has_value(protocol, v["protocol"])
            then
...
function get_name_from_index(i, notitle)
    ...
    local should_use_title = settings.prefer_titles == "all"
        or is_protocol(name) and settings.prefer_titles == "url"
...
function draw_playlist()
    ...
    if
        settings.curtain_opacity ~= nil
        and settings.curtain_opacity ~= 0
        and settings.curtain_opacity <= 1.0
    then
    ...
    while visible_indices_length < settings.showamount and visible_indices_length < plen do
        ...
        if
            above >= 1
            and visible_indices_length < settings.showamount
            and visible_indices_length < plen
        then
...
function handle_complex_playlist_toggle(table)
    ...
    elseif event == "down" then
        ...
        if settings.peek_respect_display_timeout then
            ...
            peek_display_timer =
                mp.add_periodic_timer(settings.playlist_display_timeout, peek_timeout)
        end
...
function save_playlist(filename)
    ...
    if not file then
        msg.error(
            "Error in creating playlist file, check permissions. Error: " .. (err or "unknown")
        )
    else
        file:write("#EXTM3U\n")
        local i = 0
...
function url_fetching_throttler()
    ...
    local amount_to_fetch =
        math.max(0, settings.concurrent_title_resolve_limit - ongoing_url_requests_count)
...
function resolve_ytdl_title(filename)
    ...
    local req = mp.command_native_async({
        ...
        if res.status == 0 then
            ...
            msg.error(
                "Failed to resolve url title " .. filename .. " Error: " .. (res.error or "unknown")
            )
        end
    end)
